...
class Piece:
    def __init__(self, screen: py.Surface, teamColor: int, chessCoord: tuple[int, int], board: List[List[int]], piece: str) -> None:
            ...
            self.hasMoved = False
            #load image
            img = py.image.load(f"pieces/pieceIcons/black{piece}.png").convert_alpha() if self.color==BLACK else py.image.load(f"pieces/pieceIcons/white{piece}.png").convert_alpha()
